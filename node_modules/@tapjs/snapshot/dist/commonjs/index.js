...
exports.config = exports.SnapshotPlugin = exports.plugin = void 0;
...
const clean_cwd_js_1 = require("./clean-cwd.js");
const provider_js_1 = require("./provider.js");
const defaultFormatSnapshot = (co = {}) => (obj) => (0, tcompare_1.format)(obj, { sort: true, ...co });
const plugin = (t, opts = {}) => new SnapshotPlugin(t, opts);
...
class SnapshotPlugin {
    ...
    constructor(t, opts) {
        ...
        this.#provider =
            opts.snapshotProvider || pp || provider_js_1.SnapshotProviderDefault;
        if (typeof opts.writeSnapshot === 'boolean') {
            ...
            if (p) {
                ...
                this.writeSnapshot =
                    core_1.env.TAP_SNAPSHOT === '1' ||
                        core_1.env.npm_lifecycle_event === 'snap' ||
                        core_1.env.npm_lifecycle_event === 'snapshot';
            }
        }
    ...
    matchSnapshot(found, ...[msg, extra]) {
        ...
        const me = (0, core_1.normalizeMessageExtra)('must match snapshot', args);
        const m = this.#t.fullname + ' > ' + me[0];
        if (typeof found !== 'string') {
            if (!this.#formatSnapshot) {
                for (let p = this.#t.parent; p && p.t.pluginLoaded(exports.plugin); p = p.parent) {
        ...
        if (process.env.TAP_SNAPSHOT_CLEAN_CWD !== '0') {
            found = (0, clean_cwd_js_1.cleanCWD)(found);
        }
        if (this.writeSnapshot) {
        ...
        const { diff } = (0, tcompare_1.strict)(found, wanted, this.#compareOptions);
        Object.assign(me[1], {
            found,
            wanted,
            diff,
    ...
    async resolveMatchSnapshot(fnOrPromise, ...[msg, extra]) {
        const args = [msg, extra];
        const me = (0, core_1.normalizeMessageExtra)('promise must resolve to match snapshot', args);
        let p;
        try {
            p =
    ...
    static #getFilename(t, opts) {
        ...
        const args = core_1.argv.slice(2);
        const head = (0, path_1.relative)(core_1.cwd, (0, path_1.resolve)(main));
        const tail = args.length === 0 ?
            ''
            : '-' + args.join(' ').replace(/[^a-zA-Z0-9\._\-]/g, '-');
        return (0, path_1.resolve)(core_1.cwd, 'tap-snapshots', head + tail + '.test.cjs');
    }
}
