...
exports.CallSiteLike = void 0;
const module_1 = require("module");
const path_1 = require("path");
const url_1 = require("url");
const parse_js_1 = require("./parse.js");
const methodRe = /^(.*?) \[as (.*?)\]$/;
const isCallSite = (c) => !!c && typeof c === 'object' && c.constructor?.name === 'CallSite';
class CallSiteLike {
    static prepareStackTrace(e, c) {
    ...
    get absoluteFileName() {
        if (!this.#fileName)
            ...
            return (0, url_1.fileURLToPath)(this.#fileName);
        }
        else {
            return this.#fileName;
        }
    ...
    set cwd(cwd) {
        ...
        if (this.evalOrigin)
            this.evalOrigin.cwd = cwd;
    }
    constructor(e, c) {
        if (typeof c === 'string') {
            c = (0, parse_js_1.parseCallSiteLine)(c);
        }
        if (isCallSite(c)) {
        ...
        else if ((0, parse_js_1.isCompiledCallSiteLine)(c)) {
        ...
        if (this.#fileName &&
            ...
            /* c8 ignore stop */
            this.#sourceMap = (0, module_1.findSourceMap)(this.#fileName, sme);
            if (this.#sourceMap && typeof this.lineNumber === 'number') {
                ...
                /* c8 ignore start */
                Math.max(0, this.lineNumber - 1), Math.max(0, (this.columnNumber || 0) - 1));
                if (payload) {
                    const offset = [
    ...
    toJSON() {
        const { fileName, lineNumber, columnNumber, evalOrigin, typeName, methodName, functionName, isEval, isNative, isToplevel, isConstructor, generated, } = this;
        const json = {};
        if (fileName !== null)
        ...
        if (evalOrigin)
            json.evalOrigin = evalOrigin.toJSON();
        if (typeName !== null) {
        ...
        if (isToplevel)
            json.isToplevel = isToplevel;
        if (isConstructor)
            json.isConstructor = isConstructor;
...
exports.CallSiteLike = CallSiteLike;
//# sourceMappingURL=call-site-like.js.map