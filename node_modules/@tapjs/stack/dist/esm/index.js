 ...
 */
export const setFilterNodeInternals = (s) => (filterNodeInternals = s);
 ...
 * from stack traces.
 */
export const addIgnoredPackage = (s) => {
 ...
 * from stack traces.
 */
export const removeIgnoredPackage = (s) => {
    const i = ignoredPackages.indexOf(s);
    if (i !== -1) {
 ...
 */
export const setFilterIgnoredPackages = (s) => (filterIgnoredPackages = s);
/**
...
let capturing = false;
export function capture(limit = 0, fn = capture) {
 ...
 */
export const at = (fn = at) => {
    const [site] = capture(1, fn);
    return site;
};
export function captureString(limit = Infinity, fn = captureString) {
    if (typeof limit === 'function') {
        fn = limit;
        limit = 0;
    }
...
export const captureError = (e) => {
    ...
    const s = cleanHead ? stack.substring(head.length)
 ...
 * weird results.
 */
export const captureErrorString = (e) => captureError(e)
    .map(c => String(c) + '\n')
    .join('');
/**
 ...
 * stack.
 */
export const parseStack = (s) => clean(s
    .trimEnd()
    .split('\n')
    .filter(l => !!l.trim())
